...
//     }
    ...
    private int loginDevice()
    {
        ...
        if (iLogID < 0)
        {
            Log.e("login", "NET_DVR_Login is failed!Err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return -1;
        }
...
//        }
        ...
        if (m_iPlayID < 0)
        {
            Log.e("play", "NET_DVR_RealPlay is failed!Err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return ;
        }
...
//异常回调
    ...
    private RealPlayCallBack getRealPlayerCbf()
    {
        RealPlayCallBack cbf = new RealPlayCallBack()
        {
            ...
            {
                // player channel 1
                MainActivity.this.processRealData(1, iDataType, pDataBuffer, iDataSize, Player.STREAM_REALTIME);
            }
        };
        return cbf;
    }
    ...
    public void processRealData(int iPlayViewNo, int iDataType, byte[] pDataBuffer, int iDataSize, int iStreamMode)
    {
            ...
            if(HCNetSDK.NET_DVR_SYSHEAD == iDataType)
            {
                ...
                if(m_iPort == -1)
                {
                    Log.e("play", "getPort is failed with: " + Player.getInstance().getLastError(m_iPort));
                    return;
                }
                ...
                if (iDataSize > 0)
                {
                    ...
                    {
                        Log.e("play", "playSound failed with error code:" + Player.getInstance().getLastError(m_iPort));
                        return;
                    }
                }
            }
...
//		    		Log.e(TAG, "inputData failed with: " + Player.getInstance().getLastError(m_iPort));
                    ...
                    {
                        if (!Player.getInstance().inputData(m_iPort, pDataBuffer, iDataSize))
                            Log.e("play", "inputData failed with: " + Player.getInstance().getLastError(m_iPort));
                        else
                            break;
                        try {
                            Thread.sleep(10);
